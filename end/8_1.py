...
def SVD(matrix):
    ...
    U = np.array([ev[1] for ev in eigenvectors])
    ...
    # Construct matrix V
    V = np.array([np.dot(transpose_matrix, ev[1]) for ev in eigenvectors])
...
def PD(matrix):
    ...
    # Construct matrix P
    P = np.dot(V, np.dot(np.diag(Sigma), V.T))
