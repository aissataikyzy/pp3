...
def engineering_calculator(key):
    if key == "±":
        if "=" in calc.get():
            calc.delete(0, tk.END)
        try:
...
def make_operation_button(operation):
    return tk.Button(text=operation, bd=5, font=('Arial', 13), fg='orange'
...
win = tk.Tk()
win.geometry(f"500x270+100+200")
win['bg'] = '#000000'
...
#engineering_calculator('±').grid(row=2, column=10, stick='wens', padx=5, pady=5)
